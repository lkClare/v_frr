			      ...
			      struct bgp_path_info *bpi2)
{
	...
	if (!compare && bpi1->attr->nh_type != NEXTHOP_TYPE_BLACKHOLE) {
		if (bpi1->extra && bpi1->extra->pvrfleak && bpi1->extra->pvrfleak->bgp_orig && bpi2->extra
		    && bpi2->extra->pvrfleak && bpi2->extra->pvrfleak->bgp_orig) {
			if (bpi1->extra->pvrfleak->bgp_orig->vrf_id
			    != bpi2->extra->pvrfleak->bgp_orig->vrf_id) {
				compare = 1;
			}
		}
	}
