			    ...
			    json_object *json_paths)
{
	...
	if (display == NLRI_STRING_FORMAT_LARGE) {
		...
		if (extra->pfs && extra->pfs->bgp_fs_pbr && listcount(extra->pfs->bgp_fs_pbr)) {
			...
			for (ALL_LIST_ELEMENTS_RO(extra->pfs->bgp_fs_pbr,
						  node, bpme)) {
		...
		if (extra->pfs && extra->pfs->bgp_fs_iprule && listcount(extra->pfs->bgp_fs_iprule)) {
			struct listnode *node;
			struct bgp_pbr_rule *bpr;

			if (!list_began)
				vty_out(vty, "\tinstalled in PBR");
			for (ALL_LIST_ELEMENTS_RO(extra->pfs->bgp_fs_iprule,
						  node, bpr)) {
				if (!bpr->action)
					continue;
				if (!list_began) {
