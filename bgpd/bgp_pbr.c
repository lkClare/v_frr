	...
	if (bpr->installed) {
		...
		if (bpr->path) {
			...
			if (extra->pfs && extra->pfs->bgp_fs_iprule)
				listnode_delete(extra->pfs->bgp_fs_iprule, bpr);
			bpr->path = NULL;
		}
	}
	...
	if (bpme->installed) {
		...
		if (bpme->path) {
			...
			if (extra->pfs && extra->pfs->bgp_fs_pbr)
				listnode_delete(extra->pfs->bgp_fs_pbr, bpme);
			bpme->path = NULL;
		}
	}
						  ...
						  float *rate)
{
	...
	if (bpf->type == BGP_PBR_IPRULE) {
		...
		if (bpr_found) {
			...
			if (extra && extra->pfs &&
			    listnode_lookup_nocheck(extra->pfs->bgp_fs_iprule,
						    bpr)) {
				if (BGP_DEBUG(pbr, PBR_ERROR))
	...
	if (bpme_found) {
		...
		if (extra && extra->pfs &&
		    listnode_lookup_nocheck(extra->pfs->bgp_fs_pbr, bpme)) {
			if (BGP_DEBUG(pbr, PBR_ERROR))
				zlog_err(
